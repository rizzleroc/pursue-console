...
- **Orphaned files left in place pending maintainer call** (may be regenerated by build steps): `src/data/threads.js` (no longer imported by any view), `public/visuals.json` (`{}`; visuals served via `dossier-extracts.json`), `public/coverage.json` (fetched by no view).
